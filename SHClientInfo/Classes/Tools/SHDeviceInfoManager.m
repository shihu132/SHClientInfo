...
@implementation SHDeviceInfoManager

+ (instancetype)sharedManager {
    static SHDeviceInfoManager *_manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _manager = [[SHDeviceInfoManager alloc] init];
    });
    return _manager;
}
...
- (const NSString *)getDeviceName {
    return [[SHDeviceDataLibrery sharedLibrery] getDiviceName];
}

- (const NSString *)getInitialFirmware {
    return [[SHDeviceDataLibrery sharedLibrery] getInitialVersion];
}

- (const NSString *)getLatestFirmware {
    return [[SHDeviceDataLibrery sharedLibrery] getLatestVersion];
}
...
- (NSString *)getCPUProcessor {
    return [[SHDeviceDataLibrery sharedLibrery] getCPUProcessor] ? : @"unKnown";
}
